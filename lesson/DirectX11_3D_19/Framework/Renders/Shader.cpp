	...
	: file(L"../_Shaders/" + file)
{
	...
	{
		DEVICE_CONTEXT->RSGetState(&initialStateBlock->RSRasterizerState);
		DEVICE_CONTEXT->OMGetBlendState(&initialStateBlock->OMBlendState, initialStateBlock->OMBlendFactor, &initialStateBlock->OMSampleMask);
		DEVICE_CONTEXT->OMGetDepthStencilState(&initialStateBlock->OMDepthStencilState, &initialStateBlock->OMStencilRef);
	}
...
void Shader::Pass::Draw(UINT vertexCount, UINT startVertexLocation)
{
	BeginDraw();
	{
		DEVICE_CONTEXT->Draw(vertexCount, startVertexLocation);
	}
	EndDraw();
}
...
void Shader::Pass::DrawInstanced(UINT vertexCountPerInstance, UINT instanceCount, UINT startVertexLocation, UINT startInstanceLocation)
{
	BeginDraw();
	{
		DEVICE_CONTEXT->DrawInstanced(vertexCountPerInstance, instanceCount, startVertexLocation, startInstanceLocation);
	}
	EndDraw();
}
...
void Shader::Pass::DrawIndexedInstanced(UINT indexCountPerInstance, UINT instanceCount, UINT startIndexLocation, INT baseVertexLocation, UINT startInstanceLocation)
{
	BeginDraw();
	{
		DEVICE_CONTEXT->DrawIndexedInstanced(indexCountPerInstance, instanceCount, startIndexLocation, baseVertexLocation, startIndexLocation);
	}
	EndDraw();
}
...
void Shader::Pass::BeginDraw()
{
	...
	IPass->Apply(0, DEVICE_CONTEXT);
}

void Shader::Pass::EndDraw()
{
	if (StateBlockMask.RSRasterizerState == 1)
		DEVICE_CONTEXT->RSSetState(StateBlock->RSRasterizerState);

	if (StateBlockMask.OMDepthStencilState == 1)
		DEVICE_CONTEXT->OMSetDepthStencilState(StateBlock->OMDepthStencilState, StateBlock->OMStencilRef);

	if (StateBlockMask.OMBlendState == 1)
		DEVICE_CONTEXT->OMSetBlendState(StateBlock->OMBlendState, StateBlock->OMBlendFactor, StateBlock->OMSampleMask);
...
void Shader::Pass::Dispatch(UINT x, UINT y, UINT z)
{
	...
	DEVICE_CONTEXT->CSSetShaderResources(0, 1, null);

	ID3D11UnorderedAccessView* nullUav[1] = { 0 };
	DEVICE_CONTEXT->CSSetUnorderedAccessViews(0, 1, nullUav, NULL);
...
void Shaders::GetEffect(wstring fileName, ID3DBlob** blob, ID3DX11Effect** effect)
{
	...
	if (shaders.count(fileName) < 1)
	{
		...
		hr = D3DX11CreateEffectFromMemory(p.second.blob->GetBufferPointer(), p.second.blob->GetBufferSize(), 0, DEVICE, &p.second.effect);
		Check(hr);
