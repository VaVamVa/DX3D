...
void Texture::SaveFile(wstring file, ID3D11Texture2D * src)
{
	...
	hr = D3DX11LoadTextureFromTexture(DEVICE_CONTEXT, src, NULL, dest);
	Check(hr);
	...
	if(ext == L"BMP")
		hr = D3DX11SaveTextureToFile(DEVICE_CONTEXT, dest, D3DX11_IFF_BMP, file.c_str());
	else
		hr = D3DX11SaveTextureToFile(DEVICE_CONTEXT, dest, D3DX11_IFF_PNG, file.c_str());
...
D3D11_TEXTURE2D_DESC Texture::ReadPixel(ID3D11Texture2D * src, DXGI_FORMAT readFormat, vector<Color>* pixels)
{
	...
	ID3D11Texture2D* texture;
	Check(DEVICE->CreateTexture2D(&desc, NULL, &texture));
	Check(D3DX11LoadTextureFromTexture(DEVICE_CONTEXT, src, NULL, texture));

	UINT* colors = new UINT[desc.Width * desc.Height];
	D3D11_MAPPED_SUBRESOURCE subResource;
	DEVICE_CONTEXT->Map(texture, 0, D3D11_MAP_READ, NULL, &subResource);
	{
...
TextureArray::TextureArray(vector<wstring> & names, UINT width, UINT height, UINT mipLevels)
{
	...
	for (UINT i = 0; i < textures.size(); i++)
	{
		...
		{
			D3D11_MAPPED_SUBRESOURCE subResource;
			DEVICE_CONTEXT->Map(textures[i], level, D3D11_MAP_READ, 0, &subResource);
			{
				DEVICE_CONTEXT->UpdateSubresource(textureArray, D3D11CalcSubresource(level, i, textureDesc.MipLevels), NULL, subResource.pData, subResource.RowPitch, subResource.DepthPitch);
			}
			DEVICE_CONTEXT->Unmap(textures[i], level);
		}
	}

	//Create File textures[0] -> test.png (for Test)
	//D3DX11SaveTextureToFile(DEVICE_CONTEXT, textureArray, D3DX11_IFF_PNG, L"test.png");
	...
	: position(position), width(width), height(height), sView(NULL), sProjection(NULL)
{
	...
	//Create Texture2D - RTV
	{
		...
		Check(DEVICE->CreateTexture2D(&desc, NULL, &rtvTexture));
	}
	...
	//Create RTV
	{
		...
		Check(DEVICE->CreateRenderTargetView(rtvTexture, &desc, &rtv));
	}
	...
	//Create SRV
	{
		...
		Check(DEVICE->CreateShaderResourceView(rtvTexture, &desc, &srv));
	}
	...
	//Create Texture - DSV
	{
		...
		Check(DEVICE->CreateTexture2D(&desc, NULL, &dsvTexture));
	}
	...
	//CreateDSV
	{
		...
		Check(DEVICE->CreateDepthStencilView(dsvTexture, &desc, &dsv));
	}
...
void TextureCube::Position(Vector3 & position)
{
	...
	projection = new Projection(1, 1, 0.1f, 500, Math::PI * 0.5f);
}
...
void TextureCube::Set(Shader * shader)
{
	...
	Graphics::Get()->SetRenderTarget(rtv, dsv);
	Graphics::Get()->Clear(Color(0, 0, 0, 1), rtv, dsv);
